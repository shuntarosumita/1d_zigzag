//-*- coding: utf-8; mode: c++ -*-
// Time-stamp: <2015-12-30 14:25:02 shunta>
...
namespace Zigzag {
  ...
  const vector<Complex> order_zero(2, 0.0);     // ゼロギャップ
  ...
  public:
    ...
    void init_zmatrix(vector<Complex> order, double haf, double mu, double q, double k) {
      ...
      // 1列目
      h[0][0] = nn_hopping[0] + as[0] - haf - mu;

      // 2列目
      h[1][0] = off_diag[0];
      h[1][1] = nn_hopping[0] - as[0] + haf - mu;

      // 3列目
      h[2][2] = nn_hopping[0] + as[0] - haf - mu;

      // 4列目
      h[3][2] = off_diag[0];
      h[3][3] = nn_hopping[0] - as[0] + haf - mu;

      // 5列目
      h[4][3] = - order[0];
      h[4][4] = - nn_hopping[1] - as[1] + haf + mu;

      // 6列目
      h[5][2] = - order[1];
      h[5][4] = - off_diag[1];
      h[5][5] = - nn_hopping[1] + as[1] - haf + mu;

      // 7列目
      h[6][1] = order[1];
      h[6][6] = - nn_hopping[1] - as[1] + haf + mu;

      // 8列目
      h[7][0] = order[0];
      h[7][6] = - off_diag[1];
      h[7][7] = - nn_hopping[1] + as[1] - haf + mu;
    }
  ...
  // 物理量をファイルに出力する
  void output(ofstream& physical_quantities, vector<Complex> order, double haf, double mu, double q, double T, double beta);
}
