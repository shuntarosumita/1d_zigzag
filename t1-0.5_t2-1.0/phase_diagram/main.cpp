//-*- coding: utf-8 -*-
// Time-stamp: <2016-01-08 10:56:28 shunta>
// ----------------------------------------
// 1次元ジグザグチェーン構造の超伝導計算
// ----------------------------------------
#include "zigzag.h"
...
int main(int argc, char* argv[]) {
  ...
  // データを出力するためのフォルダを作成
  mkdir("./data", 0755);
  filename << fixed << setprecision(2) << "./data/haf" << haf;
  ...
  // 出力ファイルを作成
  filename << fixed << setprecision(2) << "./data/haf" << haf << "/physical_quantities/mu" << mu << ".d";
  output_file[0].open(filename.str().c_str());
  filename.str("");                      // バッファのクリア
  filename.clear(stringstream::goodbit); // フラグのクリア

  filename << fixed << setprecision(2) << "./data/haf" << haf << "/phase_diagram/mu" << mu << ".d";
  output_file[1].open(filename.str().c_str());
  filename.str("");                      // バッファのクリア
  filename.clear(stringstream::goodbit); // フラグのクリア

  // 最初の行のコメントを挿入
  output_file[0] << "# T q order_a_ABS order_a_ARG order_b_ABS order_b_ARG free_energy(S) free_energy(N)" << endl;
  output_file[1] << "# T mu q" << endl;

  int loopcount, i, j;                   // ループで必要な変数

  // 温度を変化させながら計算・出力する
  cout << "#T haf q order_a_R order_a_I order_b_R order_b_I cond_energy" << endl;
...
#pragma omp critical
    {
      output(output_file[0], order[0], haf, mu, q_opti, T, beta);
      if( ( abs(order[0][0]) >= 1.0e-5 || abs(order[0][1]) >= 1.0e-5 ) && omega - omega_n < 0.0 ) {
        output_file[1] << fixed << setprecision(8) << T << "  " << mu << "  " << q_opti << endl;
      }
    }
