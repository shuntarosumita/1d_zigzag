//-*- coding: utf-8 -*-
// Time-stamp: <2015-12-21 19:52:54 shunta>
// ----------------------------------------
// 1次元ジグザグチェーン構造の超伝導計算
// ----------------------------------------
#include "zigzag.h"
...
int main(int argc, char* argv[]) {
  ...
  // 出力ファイルを作成
  filename << fixed << setprecision(2) << "./data/mu" << mu << "/eigen_values/BCS_h" << haf << ".d";
  output_file[0].open(filename.str().c_str());
  filename.str("");                      // バッファのクリア
  filename.clear(stringstream::goodbit); // フラグのクリア

  filename << fixed << setprecision(2) << "./data/mu" << mu << "/physical_quantities/BCS_h" << haf << ".d";
  output_file[1].open(filename.str().c_str());
  filename.str("");                      // バッファのクリア
  filename.clear(stringstream::goodbit); // フラグのクリア

  filename << fixed << setprecision(2) << "./data/mu" << mu << "/phase_diagram/BCS_h" << haf << ".d";
