//-*- coding: utf-8; mode: c++ -*-
// Time-stamp: <2016-01-06 16:12:18 shunta>
...
namespace Zigzag {
  ...
  const long DIM = 2;                           // Hamiltonianの次元
  ...
  public:
    ...
    void init_zmatrix(Complex order, double haf, double mu, double q, double k) {
      ...
      h[0][0] = next_nearest_neighbour_hopping(k) + alpha * sin(k) - haf;
      h[1][0] = nearest_neighbour_hopping(k);
      h[1][1] = next_nearest_neighbour_hopping(k) - alpha * sin(k) + haf;
    }
