//-*- coding: utf-8 -*-
// Time-stamp: <2016-01-27 13:20:25 shunta>
// ----------------------------------------
// 1次元ジグザグチェーン構造の超伝導計算
// ----------------------------------------
#include "zigzag.h"
...
int main(int argc, char* argv[]) {
  ...
  double epsilon = 0.005;                 // Lorentzianのパラメータ

  // 秩序パラメータの初期化
  vector<Complex> order_init;
  order_init.push_back(Complex(0.5, 0.0));
  order_init.push_back(Complex(-0.5, 0.0));
...
//     if( abs(arg(order[0][0]) - arg(order[0][1])) < M_PI || abs(arg(order[0][0]) - arg(order[0][1])) > 1.95 * M_PI ) {
//       order[0][1] = - order[0][1];
//     }
...
//   }
  ...
  order[0][0] = Complex(0.5, 0.0);
  order[0][1] = Complex(-0.5, 0.0);
  ...
  if(loopcount < 200) {
    ...
    cout << "# order_a order_b cond_energy" << endl;
    cout << fixed << setprecision(8) << order[0][0] << "  " << order[0][1] << "  " << omega - omega_n << endl;
    ...
    for(i = 0; i <= N; i++) {
      ...
      // 固有エネルギーを出力する
      for(j = 0; j <= DIM - 1; j++) {
        output_file[0] << fixed << setprecision(8) << "  " << zmatrix.e[j];
      }
      output_file[0] << endl;
      ...
      for(j = 0; j <= DIM - 1; j++) {
        for(enecount = 0; enecount < max_enecount; enecount++) {
          dosS[enecount][1] += N_inv / M_PI * epsilon / (pow(dosS[enecount][0] - zmatrix.e[j], 2.0) + pow(epsilon, 2.0));
        }
      }
    ...
    for(i = 0; i <= N; i++) {
      ...
      for(j = 0; j <= DIM - 1; j++) {
        for(enecount = 0; enecount < max_enecount; enecount++) {
          dosN[enecount][1] += N_inv / M_PI * epsilon / (pow(dosS[enecount][0] - zmatrix.e[j], 2.0) + pow(epsilon, 2.0));
        }
      }
