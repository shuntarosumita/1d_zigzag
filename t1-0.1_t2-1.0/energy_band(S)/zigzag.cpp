//-*- coding: utf-8; mode: c++ -*-
// Time-stamp: <2015-12-21 15:56:46 shunta>
...
vector<Complex> Zigzag::calculate_order_parameter(vector<Complex> order_o, double haf, double mu, double q, double T, double beta) {
...
#endif
  vector<Complex> order_n(2, Complex(0.0, 0.0)); // 秩序パラメータ(固有ベクトルから計算する)
  int i, j;                                  // ループ用変数
  ...
  for(i = 0; i <= N; i++) {
    ...
    for(j = 0; j < DIM; j++) {
      // order_n[0] += - N_inv * V[0] * conj(zmatrix.h[j][7]) * zmatrix.h[j][0] * fermi(zmatrix.e[j], beta);
      // order_n[1] += - N_inv * V[1] * conj(zmatrix.h[j][6]) * zmatrix.h[j][1] * fermi(zmatrix.e[j], beta);
...
void Zigzag::output(ofstream& eigen_values, ofstream& physical_quantities, vector<Complex> order, double haf, double mu, double q, double T, double beta) {
  ...
  // 各種物理量を出力する
  physical_quantities << fixed << setprecision(8) << T << "  " << q << "  " << abs(order[0]) << "  " << arg(order[0]) << "  " << abs(order[1]) << "  " << arg(order[1]) << "  " << omega << "  " << omega_n << endl;
